...
export const call_LLM = async (prompt: string, model: LLM_MODEL = LLM_MODEL.LLAMA_3_1) => {
    ...
    try {
        ...
        type OLLAMA_RES = {response: string}
...
const SHELL_EXECUTOR_BRANCH_SIMPLIFIER_PROMPTS_FILE = "./prompts/shell_executor_branch_simplifier.md"
...
export const PROMPTS = {
    ...
    "shell_executor_branch_simplifier" : await load_context(SHELL_EXECUTOR_BRANCH_SIMPLIFIER_PROMPTS_FILE),
    // shell results
...
export const call_llm_shell_branch_analyzer = async (prompt: string) => {
    const withContext = PROMPTS['shell_executor_branch_analyst'] + PROMPTS['shell_executor_branch_analysis_types'] + prompt;
    const res = await call_LLM(withContext) 
    if (!res) throw new Error(`did not receive response from llm: ${res}`)
