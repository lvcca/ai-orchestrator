...
export const getConnection = async () => {
	try {
		...
		logger.error(
			`something went wrong in getConnection: ${e}`,
		);
		throw new Error(
			`redis connection cannot be established: error ${e}`,
		);
	}
};
...
export const redisSet = async (
	...
	try {
		...
		if (await redisExists(obj.id)) {
			const _existing_entry = await redisGet(
				obj.id,
			);
			_new_entry = {
				..._existing_entry,
		...
		logger.error(
			`something went wrong in redisSet: ${e}`,
		);
	}
...
export const redisGetAll = async () => {
	try {
		...
		if (!keys || keys.length < 1)
			throw new Error(
				`could not find entries..`,
			);

		return keys;
	} catch (e) {
		logger.error(
			`something went wrong in redisGet: ${e}`,
		);
	}
};
...
export const redisExists = async (
	...
	try {
		const _connection =
			await getConnection();
		exists =
			(await _connection.exists(id)) === 1;
	} catch (e) {
		logger.error(
			`something went wrong in redisExists: ${e}`,
		);
	}
